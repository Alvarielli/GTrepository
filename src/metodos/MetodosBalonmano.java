...
/**
 * Válido también para Waterpolo, hockey hierba
 * Se diferencian del fútbol en que las victorias puntuan como 2 puntos
 * Recibe objetos EquipoFutbol (que si se adaptan a este deporte)
 * @author Alvarielli
 */
public class MetodosBalonmano extends Metodos{
    ...
    private RenderFutbol render;//Balonmano puede usar los Renders del Fútbol
    ...
    private int[] numc = {9, 28, 9, 9, 9, 9, 9, 9, 9};//Deben sumar 100 siempre
    ...
    private final int CODIGO_ANCHO = 35;
    private final int CODIGO_ALTO = 4;
    private final int CODIGO_ANCHO_CLASF = 42;
    private final int CODIGO_ALTO_CLASF = 5;
    
    //En el constructor de clase le pasamos todo lo requerido para esta clase desde Principal
    public MetodosBalonmano(DefaultTableModel modelo, DefaultTableModel clasificacion, int jornada, JTable TableJornada, JTable TableClasificacion, JLabel LabelEquipoDescansa,
    ...
    public void cargarTabla(int jornada){
        ...
        String[] columnasLocal = {"FECHA", "HORA", "EQUIPO LOCAL", "RES.L", "RES.V", "EQUIPO VISITANTE", "PISTA"};
     ...
     public void asignarDatos(EquipoFutbol equipo, String local, int golesL, int golesV, String visitante){
         ...
         if(golesL < 0 || golesV < 0){
            ...
            if(equipo.getNombre().equalsIgnoreCase(local)){
               //Si gana el equipo local le sumamos 2 puntos
               if(golesL > golesV){
               ...
               //Si gana el equipo visitante le sumamos 2 puntos
               if(golesV > golesL){
                   equipo.setPtos(equipo.getPtos() + 2);
                   equipo.setPg(equipo.getPg() + 1);
    ...
    public void informeJornada(String rutaPDF, String nombreArchivo, boolean aplazados, ArrayList<Partido> partidosAplazados){
       ...
       if(aplazados){
        ...
        try {
            ...
            Logger.getLogger(MetodosBalonmano.class.getName()).log(Level.SEVERE, null, ex);
        }
