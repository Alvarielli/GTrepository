...
/**
 * Válido para Béisbol y Curling
 * @author Alvarielli
 */

import datasources.DataSourceEliminatoriasBeisbolResultados;
...
import renders.RenderEliminatoriaBeisbol;
import renders.RenderEliminatoriaCurling;
import java.util.Arrays;


public class MetodosEliminatoriaBeisbol extends Metodos{
    ...
    private RenderEliminatoriaBeisbol render;
    private RenderEliminatoriaCurling renderC;
    private int[] num = {7, 3, 8, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 4, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 4, 8, 5, 7, 6};// Definimos los anchos para las tablas en porcentaje (para que el usuario pueda modificarlas luego)
    ...
    private final int CODIGO_ANCHO = 275;
    private final int CODIGO_ALTO = 27;
    ...
    public MetodosEliminatoriaBeisbol(DefaultTableModel modelo, int jornada, JTable TableJornada,JLabel LabelJornada, JLabel LabelTotalJornadas, 
    ...
    public void cargarTabla(int jornada){
        ...
        if(deporte == 18){
            columnasLocal = new String[] {"FECHA", "HORA", "LOCAL", 
                ...
                "VISITANTE", "CAMPO", "GANADOR", "DESEMPATE"};
            setColumnas(columnasLocal);
        }else if(deporte == 19){
            columnasLocal = new String[] {"FECHA", "HORA", "LOCAL", 
                "L1", "L2", "L3", "L4", "L5", "L6", "L7", "L8", "L9", "L10", "L11", "L12", "RES.L",
                "V1", "V2", "V3", "V4", "V5", "V6", "V7", "V8", "V9", "V10", "V11", "V12", "RES.V", 
                "VISITANTE", "PISTA", "GANADOR", "DESEMPATE"};
            setColumnas(columnasLocal);
        }
        //Indicamos que columnas serán editables
        final boolean[] canEdit = new boolean [] {true, true, false, true, true, true, true, true, true, true, true, true, true, 
        ...
        //Definimos los anchos de las columnas y los asignamos como tamaño preferente
         int[] anchos = {(anchoTabla*num[0])/100, (anchoTabla*num[1])/100, (anchoTabla*num[2])/100, 
                        ...
                        (anchoTabla*num[27])/100, (anchoTabla*num[28])/100, (anchoTabla*num[29])/100, (anchoTabla*num[30])/100, (anchoTabla*num[31])/100, (anchoTabla*num[32])/100};
        ...
        asignarGanador(TableJornada, TableJornada.getColumnModel().getColumn(31));
        //Le asignamos el render al calendario
        if(deporte == 18){
            render = new RenderEliminatoriaBeisbol();
            getTableJornada().setDefaultRenderer(Object.class, render);
        }else if(deporte == 19){
            renderC = new RenderEliminatoriaCurling();
    ...
    public void guardarTabla(){
       ...
       for (int i = 0; i < filas; i++) {
           //Obtenemos el partido en el ArrayList y le asignamos los nuevos datos
            PartidoEliminatoriaBeisbol P = (PartidoEliminatoriaBeisbol) J.partidos.get(i);
            ...
            P.setRV11(validarEnteros(getModelo().getValueAt(i,26).toString().trim()));//Se llaman goles porque la tabla de calendario se mantienen intacta (pero son ptos)
            P.setRV12(validarEnteros(getModelo().getValueAt(i,27).toString().trim()));
            ...
            P.setGanador(limitarCadena(getModelo().getValueAt(i,31).toString().trim(), 30, false));
            P.setDesempate(limitarCadena(getModelo().getValueAt(i,32).toString().trim(), 30, false));
        }
     ...
     public void guardarXML(String ruta, String nombreArchivo){
        ...
        for (int i = 0; i < num_jornadas; i++) {          
            ...
            Jornada J = getCalendario().jornadas.get(i);//i porque es un índice
                ...
                //Ahora usamos el for-each y obtenemos la lista de partidos de este objeto Jornada
                for (PartidoEliminatoriaBeisbol partido :(ArrayList<PartidoEliminatoriaBeisbol>) J.getListaPartidos()){ 
    ...
    public void informeJornada(String rutaPDF, String nombreArchivo, boolean aplazados, ArrayList<Partido> partidosAplazados){
       ...
       if(aplazados){
        ...
        try {
            ...
            Logger.getLogger(MetodosEliminatoriaBeisbol.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public void informeResultados(String rutaPDF, String nombreArchivo, boolean aplazados, ArrayList<Partido> partidosAplazados){
       ...
       HashMap<String, Object> parametros = new HashMap<>();
       String descansa = "";//Obtenemos el nombre del equipo que descansa        
       DataSourceEliminatoriasBeisbolResultados datasource = new DataSourceEliminatoriasBeisbolResultados();  
       if(aplazados){
            //Añadimos los partidos aplazados al datasource
             for (int i = 0; i < partidosAplazados.size(); i++) {
                PartidoEliminatoriaBeisbol P = (PartidoEliminatoriaBeisbol) partidosAplazados.get(i);
                datasource.addPartido(P);
             }
       }else{
            Jornada J = getCalendario().jornadas.get(getJornada() - 1);//coge la jornada como variable global, -1 porque es un índice
        ...
        File archivoInforme; 
        if(deporte == 18){
            archivoInforme = new File("src/informes/eliBeisbolResultados.jasper");
        }else{
            archivoInforme = new File("src/informes/eliCurlingResultados.jasper");
        }
        JasperReport reporte;
        parametros.put("nombreTorneo", nombreTorneo);
        if(aplazados){
