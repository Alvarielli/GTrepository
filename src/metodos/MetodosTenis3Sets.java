...
import datasources.DataSourceTenis3SetsResultados;
import datasources.DataSourceTenisClasificacion;
...
import renders.RenderTenis3S;
import java.util.Arrays;
import java.util.Map;


/**
 * Válido también para Badminton*(se cambio al final un pequeño detalle y se usa MetodosBadminton), Frontenis, Pádel y Voley Playa
 * @author Alvarielli
 */
public class MetodosTenis3Sets extends Metodos{   
    ...
    private EquipoLista <EquipoTenis> castEquipoLista;//EquipoLista con los Equipos ya casteados en Principal (seleccionarClaseEquipo)
    ...
    private RenderTenis3S render;
    private RenderTenisClasf renderClasf;
    private int[] num = {10, 9, 24, 4, 4, 4, 4, 4, 4, 24, 9};// Definimos los anchos para las tablas en porcentaje (para que el usuario pueda modificarlas luego)
    private int[] numc = {8, 36, 8, 8, 8, 8, 8, 8, 8};//Deben sumar 100 siempre
    ...
    private final int CODIGO_ANCHO = 173;
    private final int CODIGO_ALTO = 18;
    private final int CODIGO_ANCHO_CLASF = 184;
    private final int CODIGO_ALTO_CLASF = 19;
    
    //En el constructor de clase le pasamos todo lo requerido para esta clase desde Principal
    public MetodosTenis3Sets(DefaultTableModel modelo, DefaultTableModel clasificacion, int jornada, JTable TableJornada, JTable TableClasificacion, JLabel LabelEquipoDescansa,
    ...
    public void cargarTabla(int jornada){
        ...
        String[] columnasLocal = {"FECHA", "HORA", "LOCAL", "SET 1 (L)", "SET 1 (V)", "SET 2 (L)", "SET 2 (V)", "SET 3 (L)", "SET 3 (V)", "VISITANTE", "PISTA"};
        setColumnas(columnasLocal);
        //Indicamos que columnas serán editables
        final boolean[] canEdit = new boolean [] {true, true, false, true, true, true, true, true, true, false, true};
        ...
        //Definimos los anchos de las columnas y los asignamos como tamaño preferente
         int[] anchos = {(anchoTabla*num[0])/100, (anchoTabla*num[1])/100, (anchoTabla*num[2])/100, (anchoTabla*num[3])/100, (anchoTabla*num[4])/100, (anchoTabla*num[5])/100,
             (anchoTabla*num[6])/100, (anchoTabla*num[7])/100, (anchoTabla*num[8])/100, (anchoTabla*num[9])/100, (anchoTabla*num[10])/100};
    ...
    public void guardarTabla(){
       ...
       for (int i = 0; i < filas; i++) {
           ...
           //Obtenemos el partido en el ArrayList y le asignamos los nuevos datos
            PartidoTenis3Sets P = (PartidoTenis3Sets) J.partidos.get(i);
            ...
            P.setVisitante(limitarCadena(getModelo().getValueAt(i,9).toString().trim(), 40, false));
            P.setPista(limitarCadena(getModelo().getValueAt(i,10).toString().trim(), 30, false));
    ...
    @Override
     public void cargarClasificacion(){
        //Textos para las columnas
        String[] columnasLocal = {"POSICIÓN", "COMPETIDOR", "PJ", "PG", "PP", "SF", "SC", "PF", "PC"};
        ...
        int[] anchos = {(anchoTabla*numc[0])/100, (anchoTabla*numc[1])/100, (anchoTabla*numc[2])/100, (anchoTabla*numc[3])/100, (anchoTabla*numc[4])/100, (anchoTabla*numc[5])/100, (anchoTabla*numc[6])/100, (anchoTabla*numc[7])/100, (anchoTabla*numc[8])/100};
    ...
    @Override
     public void calcularClasificacion(){
         ...
         int set1L, set1V, set2L, set2V, set3L, set3V;
            ...
            for (int i = 0; i < filas; i++) {
                //Obtenemos el partido en el ArrayList y sus datos correspondientes
                 ...
                 for (EquipoTenis equipo: (ArrayList<EquipoTenis>)getCastEquipoLista().getEquipos()){                    
                     ...
                     asignarDatos(equipo, local, set1L, set1V, set2L, set2V, set3L, set3V, visitante);
    ...
    @Override
      ...
      * @param visitante 
      */
     public void asignarDatos(EquipoTenis equipo, String local, int set1L, int set1V, int set2L, int set2V, int set3L, int set3V, String visitante){
         ...
         if(set1L < 0 || set1V < 0){
             ...
             if((set1L > set1V && set2L > set2V) || (set1V > set1L && set2V > set2L)){ 
                ...
                if(equipo.getNombre().equalsIgnoreCase(local)){
                  ...
                  if(set1L > set1V){
                      ...
                      equipo.setJf(equipo.getJf() + set1L);
                      equipo.setJc(equipo.getJc() + set1V);
                  }else if(set1L < set1V){
                  ...
                  if(set2L > set2V){
                      equipo.setSf(equipo.getSf() + 1);
                      equipo.setJf(equipo.getJf() + set2L);
                      equipo.setJc(equipo.getJc() + set2V);
                  }else if(set2L < set2V){
                      //Sumamos a partidos perdidos
                      equipo.setSc(equipo.getSc() + 1);
                      equipo.setJf(equipo.getJf() + set2L);
               ...
               if(set1V > set1L){
                   equipo.setSf(equipo.getSf() + 1);
                   equipo.setJf(equipo.getJf() + set1V);
                   equipo.setJc(equipo.getJc() + set1L);
               }else if(set1V < set1L){
                   //Sumamos a partidos perdidos
                   equipo.setSc(equipo.getSc() + 1);
                   equipo.setJf(equipo.getJf() + set1V);
               ...
               if(set2V > set2L){
                   equipo.setSf(equipo.getSf() + 1);
                   equipo.setJf(equipo.getJf() + set2V);
                   equipo.setJc(equipo.getJc() + set2L);
               }else if(set2V < set2L){
                   //Sumamos a partidos perdidos
                   equipo.setSc(equipo.getSc() + 1);
                   equipo.setJf(equipo.getJf() + set2V);
           ...
           }//End if equipos no seleccionado
          /**** SI HAY TERCER SET SUMAMOS LOS DATOS DE LOS 3 SETS (NO SÓLO DE 2) ***/     
          }else{
             //Ahora tenemos que establecer la diferencia de ptos. que acumula cada equipo
                if(equipo.getNombre().equalsIgnoreCase(local)){
                    ...
                    if(set3L > set3V){
                        ...
                        equipo.setSf(equipo.getSf() + 1);
                        equipo.setJf(equipo.getJf() + set3L);
                        equipo.setJc(equipo.getJc() + set3V);
                        equipo.setPg(equipo.getPg() + 1);
                        equipo.setPj(equipo.getPj() + 1);
                    }else if(set3L < set3V){
                        //Sumamos a partidos perdidos
                    ...
                    if(set3V > set3L){
                        //Sólo si se suma si se juega
                        equipo.setSf(equipo.getSf() + 1);
                        equipo.setJf(equipo.getJf() + set3V);
                        equipo.setJc(equipo.getJc() + set3L);
                        equipo.setPg(equipo.getPg() + 1);
                        equipo.setPj(equipo.getPj() + 1);
                    }else if(set3V < set3L){
                        //Sumamos a partidos perdidos
    ...
    @Override
     public void ordenarClasificacion(){         
        ...
        Collections.sort(getCastEquipoLista().getEquipos(), new Comparator<EquipoTenis>() {
            @Override
            public int compare(EquipoTenis e1, EquipoTenis e2) {
                    // Aqui esta el truco, ahora comparamos p2 con p1 y no al reves como antes
                    return new Integer(e2.getJf()).compareTo(e1.getJf());
            }                                        
        });
         //Le indicamos al comparador que el tipo de objeto a ordenar son Equipos
        Collections.sort(getCastEquipoLista().getEquipos(), new Comparator<EquipoTenis>() {
            ...
            public int compare(EquipoTenis e1, EquipoTenis e2) {
                    // Aqui esta el truco, ahora comparamos p2 con p1 y no al reves como antes
                    return new Integer(e2.getJf() - e2.getJc()).compareTo(e1.getJf() - e1.getJc());
            }                                        
        });
        ...
        Collections.sort(getCastEquipoLista().getEquipos(), new Comparator<EquipoTenis>() {
            ...
            public int compare(EquipoTenis e1, EquipoTenis e2) {
                    if(e2.getPg() == e1.getPg()){
                        ...
                        return new Integer(e1.getPj()).compareTo(e2.getPj());
                    }
                    // Aqui esta el truco, ahora comparamos p2 con p1 y no al reves como antes
    ...
    public void informeJornada(String rutaPDF, String nombreArchivo, boolean aplazados, ArrayList<Partido> partidosAplazados){
       ...
       if(aplazados){
        ...
        File archivoInforme = new File("src/informes/futbolJornadaPista.jasper");
        ...
        try {
            ...
            Logger.getLogger(MetodosTenis3Sets.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public void informeResultados(String rutaPDF, String nombreArchivo, boolean aplazados, ArrayList<Partido> partidosAplazados){
       ...
       String jornadaText = valueOf(jornada);//Pasamos a String el número de jornada
       String descansa = "";//Obtenemos el nombre del equipo que descansa        
       DataSourceTenis3SetsResultados datasource = new DataSourceTenis3SetsResultados();  
       if(aplazados){
            //Añadimos los partidos aplazados al datasource
             for (int i = 0; i < partidosAplazados.size(); i++) {
                PartidoTenis3Sets P = (PartidoTenis3Sets) partidosAplazados.get(i);
                datasource.addPartido(P);
             }
             jornadaText = "Aplz";//Cambiamo el nombre de la jornada a Aplazados
        }else{
        ...
        File archivoInforme = new File("src/informes/tenis3SResultados.jasper");
        JasperReport reporte;
        parametros.put("nombreTorneo", nombreTorneo);
        parametros.put("jornada", jornadaText);
        //Si el número de equipos es impar añadimos el texto DESCANSA: y el nombre del equipo que descansa al informe, de lo contrario mejor que no aparezca
    ...
    public void informeClasificacion(String rutaPDF, String nombreArchivo){
        //lo primero de todo nos aseguramos de que ha guardado los datos de la tabla
       this.guardarTabla();
       ArrayList<EquipoTenis> equipos = castEquipoLista.getEquipos();
        DataSourceTenisClasificacion datasource = new DataSourceTenisClasificacion();  
        //Añadimos los partidos de la jornada al datasource
        for (EquipoTenis equipo : equipos) {
            EquipoTenis E = (EquipoTenis) equipo;
            //Comprobamos si descansa algún equipo y guardamos su nombre en la variable indicada
        ...
        File archivoInforme = new File("src/informes/tenisClasificacion.jasper");
        JasperReport reporte;
        //Creamos un HashMap para poder utilizar los parámetros en el informe
        HashMap<String, Object> parametros = new HashMap<>();
        parametros.put("nombreTorneo", nombreTorneo);
