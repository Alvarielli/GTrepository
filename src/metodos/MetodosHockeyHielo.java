...
import datasources.DataSourceHHieloClasificacion;
import datasources.DataSourceHHieloResultados;
...
import renders.RenderHockeyHielo;
import renders.RenderHockeyHieloClasf;
import java.util.Arrays;
import java.util.Map;

/**
 * En HockeyHielo el ganar suma 2 puntos, el ganar en prórroga también, perder en la prórroga suma 1 punto y perder 0 puntos (puede haber ED enfrentamiento directo)
 * @author Alvarielli
 */
public class MetodosHockeyHielo extends Metodos{
    ...
    private RenderHockeyHielo render;
    private RenderHockeyHieloClasf renderClasf;
    private int[] num = {12, 8, 22, 8, 8, 22, 12, 8};// Definimos los anchos para las tablas en porcentaje (para que el usuario pueda modificarlas luego)
    ...
    private final int CODIGO_ANCHO = 135;
    private final int CODIGO_ALTO = 14;
    private final int CODIGO_ANCHO_CLASF = 143;
    private final int CODIGO_ALTO_CLASF = 15;
    
    //En el constructor de clase le pasamos todo lo requerido para esta clase desde Principal
    public MetodosHockeyHielo(DefaultTableModel modelo, DefaultTableModel clasificacion, int jornada, JTable TableJornada, JTable TableClasificacion, JLabel LabelEquipoDescansa,
    ...
    public void cargarTabla(int jornada){
        ...
        String[] columnasLocal = {"FECHA", "HORA", "EQUIPO LOCAL", "RES.L", "RES.V", "EQUIPO VISITANTE", "PISTA", "PRÓRROGA"};
        setColumnas(columnasLocal);
        //Indicamos que columnas serán editables
        final boolean[] canEdit = new boolean [] {true, true, false, true, true, false, true, true};
        ...
        //Definimos los anchos de las columnas y los asignamos como tamaño preferente
         int[] anchos = {(anchoTabla*num[0])/100, (anchoTabla*num[1])/100, (anchoTabla*num[2])/100, (anchoTabla*num[3])/100, (anchoTabla*num[4])/100, (anchoTabla*num[5])/100, (anchoTabla*num[6])/100, (anchoTabla*num[7])/100};
        ...
        for (Partido partido : (ArrayList<PartidoHockeyHielo>)J.getListaPartidos())
        ...
        //Indicamos a la tabla cual es su modelo y le indicamos que la última columna utiliza un JComboBox
        getTableJornada().setModel(getModelo());
        asignarProrroga(TableJornada, TableJornada.getColumnModel().getColumn(7));
        //Le asignamos el render al calendario
        render = new RenderHockeyHielo();
    ...
    public void guardarTabla(){
       ...
       for (int i = 0; i < filas; i++) {
           ...
           //Obtenemos el partido en el ArrayList y le asignamos los nuevos datos
            PartidoHockeyHielo P = (PartidoHockeyHielo)J.partidos.get(i);
            ...
            P.setProrroga(limitarCadena(getModelo().getValueAt(i,7).toString().trim(), 30, false));
            ...
            if(P.getGolesL() == P.getGolesV() && !getModelo().getValueAt(i,3).toString().equalsIgnoreCase("") && !getModelo().getValueAt(i,4).toString().equalsIgnoreCase("")){
                P.setGolesL(-1);
                P.setGolesV(-1);
                ...
                //Si la prórroga contiene un valor diferente a SI o NO !(condicion) la condición entre paréntesis y la negación fuera
            }else if(!(getModelo().getValueAt(i,7).toString().equalsIgnoreCase("SI") || getModelo().getValueAt(i,7).toString().equalsIgnoreCase("NO"))){
                P.setGolesL(-1);
                P.setGolesV(-1);
                P.setProrroga("NO");
                //Mostramos un mensaje de error en caso de que el usuario coloque un empate en la tabla
                        if(!this.mensaje){//if esclusivamente para mostrar o no el mensaje
                             JOptionPane.showMessageDialog(TableJornada, "La prorroga solo puede contener SI o NO", "Error al insertar los datos de la tabla", JOptionPane.ERROR_MESSAGE, null);
                             this.mensaje = true;
                         }
                        //Si hay prórroga (SI) y la diferencia de goles L-V o V-L es mayor que 1 mostramos el mensaje de error y vaciamos los goles
            }else if(getModelo().getValueAt(i,7).toString().equalsIgnoreCase("SI") && ((P.getGolesL() - P.getGolesV()) > 1 || (P.getGolesV() - P.getGolesL()) > 1)){
                P.setGolesL(-1);
                P.setGolesV(-1);
                //Mostramos un mensaje de error en caso de que el usuario coloque un empate en la tabla
                if(!this.mensaje){//if esclusivamente para mostrar o no el mensaje
                     JOptionPane.showMessageDialog(TableJornada, "La victoria en la prórroga sólo puede ser de un gol de diferencia", "Error al insertar los datos de la tabla", JOptionPane.ERROR_MESSAGE, null);
                     this.mensaje = true;
                }
            }
        }//End for
       ...
       if(!descansa.equalsIgnoreCase("")){
           ...
           PartidoHockeyHielo P = (PartidoHockeyHielo) J.partidos.get(filas);//Lo añadimos tras la carga del resto de filas en la última posición (filas es la última)
    ...
    @Override
     public void cargarClasificacion(){
        //Textos para las columnas
        String[] columnasLocal = {"POS","COMPETIDOR", "PJ", "PG", "PGP", "PPP", "PP", "GF", "GC", "PTOS"};
        ...
        renderClasf = new RenderHockeyHieloClasf();
    ...
    @Override
     public void calcularClasificacion(){
         ...
         String local, visitante, prorroga;
         int golesL, golesV;
            ...
            for (int i = 0; i < filas; i++) {
                //Obtenemos el partido en el ArrayList y sus datos correspondientes
                 ...
                 for (EquipoHockeyHielo equipo: (ArrayList<EquipoHockeyHielo>)getCastEquipoLista().getEquipos()){                    
                     ...
                     asignarDatos(equipo, local, golesL, golesV, visitante, prorroga);//Casteamos el Objeto recibido como Equipo de Futbol
                 }
             }//End for i
         }//End for j        
    ...
    @Override
      ...
      * @param prorroga 
      */
     public void asignarDatos(EquipoHockeyHielo equipo, String local, int PtosL, int PtosV, String visitante, String prorroga){
         ...
         if(PtosL < 0 || PtosV < 0){
            ...
            if(equipo.getNombre().equalsIgnoreCase(local)){
               ...
               if(PtosL > PtosV){
                   //Si hay prorroga le asignamos el partido ganado por prórroga
                   if(prorroga.equalsIgnoreCase("SI")){
                       //Si hay prórroga la diferencia de goles ha de ser de solo 1 tanto
                       if(PtosL - PtosV == 1){
                            equipo.setPgp(equipo.getPgp() + 1);
                   ...
                   }else if(prorroga.equalsIgnoreCase("NO")){
                   ...
                   if(prorroga.equalsIgnoreCase("SI")){
                       if(PtosV - PtosL == 1){
                            equipo.setPpp(equipo.getPpp() + 1);                          
                            equipo.setPtos(equipo.getPtos() + 1);//Sumamos 1 punto
                       }else{
                            //Mostramos un mensaje de error en caso de que el usuario coloque un empate en la tabla
                            if(!this.mensaje){//if esclusivamente para mostrar o no el mensaje
                                 JOptionPane.showMessageDialog(TableJornada, "La victoria en la prórroga sólo puede ser de un gol de diferencia", "Error al insertar los datos de la tabla", JOptionPane.ERROR_MESSAGE, null);
               ...
               equipo.setGf(equipo.getGf() + PtosL);
               equipo.setGc(equipo.getGc() + PtosV);
               ...
               //Sumamos los goles a favor y en contra y el partido jugado
               equipo.setPj(equipo.getPj() + 1);
               equipo.setGf(equipo.getGf() + PtosV);
               equipo.setGc(equipo.getGc() + PtosL);
      ...
      * Ordenamos en orden inverso a prioridad, primero por goles a favor, luego por diferencia de goles, y por último por puntos
      */
    @Override
     public void ordenarClasificacion(){
         //Le indicamos al comparador que el tipo de objeto a ordenar son Equipos
        Collections.sort(getCastEquipoLista().getEquipos(), new Comparator<EquipoHockeyHielo>() {
            @Override
            public int compare(EquipoHockeyHielo e1, EquipoHockeyHielo e2) {
    ...
    /**
      ...
      * @param columna 
      */
     public void asignarProrroga(JTable tabla, TableColumn columna){
    ...
    public void informeJornada(String rutaPDF, String nombreArchivo, boolean aplazados, ArrayList<Partido> partidosAplazados){
       ...
       if(aplazados){
        ...
        try {
            ...
            Logger.getLogger(MetodosHockeyHielo.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public void informeResultados(String rutaPDF, String nombreArchivo, boolean aplazados, ArrayList<Partido> partidosAplazados){
       ...
       String jornadaText = valueOf(jornada);//Pasamos a String el número de jornada
       String descansa = "";//Obtenemos el nombre del equipo que descansa        
       DataSourceHHieloResultados datasource = new DataSourceHHieloResultados();  
       if(aplazados){
            //Añadimos los partidos aplazados al datasource
             for (int i = 0; i < partidosAplazados.size(); i++) {
                PartidoHockeyHielo P = (PartidoHockeyHielo) partidosAplazados.get(i);
                datasource.addPartido(P);
             }
             jornadaText = "Aplz";//Cambiamo el nombre de la jornada a Aplazados
       }else{
        ...
        File archivoInforme = new File("src/informes/hhieloResultados.jasper");
        JasperReport reporte;
        parametros.put("nombreTorneo", nombreTorneo);
        parametros.put("jornada", jornadaText);
        //Si el número de equipos es impar añadimos el texto DESCANSA: y el nombre del equipo que descansa al informe, de lo contrario mejor que no aparezca
    ...
    public void informeClasificacion(String rutaPDF, String nombreArchivo){
        //lo primero de todo nos aseguramos de que ha guardado los datos de la tabla
       this.guardarTabla();
       ArrayList<EquipoHockeyHielo> equipos = castEquipoLista.getEquipos();
        DataSourceHHieloClasificacion datasource = new DataSourceHHieloClasificacion();  
        //Añadimos los partidos de la jornada al datasource
        for (EquipoHockeyHielo equipo : equipos) {
            EquipoHockeyHielo E = (EquipoHockeyHielo) equipo;
            //Comprobamos si descansa algún equipo y guardamos su nombre en la variable indicada
        ...
        File archivoInforme = new File("src/informes/hhieloClasificacion.jasper");
        JasperReport reporte;
        //Creamos un HashMap para poder utilizar los parámetros en el informe
        HashMap<String, Object> parametros = new HashMap<>();
        parametros.put("nombreTorneo", nombreTorneo);
    ...
    public void informeCalendario(String rutaPDF, String nombreArchivo){
        ...
        try {
            ...
            if(num_partidos > 40){//40 es el limite de campos que le hemos puesto al page break en el futbolCalendario.jasper
                archivoInforme = new File("src/informes/hhieloCalendarioBIG.jasper");
            }else{
                archivoInforme = new File("src/informes/hhieloCalendario.jasper");
            }
            JasperReport reporte;
            //Creamos un HashMap para poder utilizar los parámetros en el informe
            HashMap<String, Object> parametros = new HashMap<>();
