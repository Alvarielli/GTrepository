...
import datasources.DataSourceRugbyClasificacion;
import datasources.DataSourceRugbyResultados;
...
import renders.RenderRugby;
import renders.RenderRugbyClasf;
 ...
 * @author Alvarielli
 */
public class MetodosRugby extends Metodos{
//Le pasamos los valores de los objetos de Principal a esta clase
    ...
    private RenderRugby render;
    private RenderRugbyClasf renderClasf;
    private int[] num = {12, 8, 18, 8, 8, 18, 8, 8, 12};// Definimos los anchos para las tablas en porcentaje (para que el usuario pueda modificarlas luego)
    private int[] numc = {7, 30, 7, 7, 7, 7, 7, 7, 7, 7, 7};//Deben sumar 100 siempre
    ...
    private final int CODIGO_ANCHO = 153;
    private final int CODIGO_ALTO = 16;
    private final int CODIGO_ANCHO_CLASF = 162;
    private final int CODIGO_ALTO_CLASF = 17;
    
    //En el constructor de clase le pasamos todo lo requerido para esta clase desde Principal
    public MetodosRugby(DefaultTableModel modelo, DefaultTableModel clasificacion, int jornada, JTable TableJornada, JTable TableClasificacion, JLabel LabelEquipoDescansa,
    ...
    public void cargarTabla(int jornada){
        ...
        String[] columnasLocal = {"FECHA", "HORA", "EQUIPO LOCAL", "RES.L", "RES.V", "EQUIPO VISITANTE", "TRIES LOCAL", "TRIES VISITANTE", "CAMPO"};
        setColumnas(columnasLocal);
        //Indicamos que columnas serán editables
        final boolean[] canEdit = new boolean [] {true, true, false, true, true, false, true, true, true};
        ...
        //Definimos los anchos de las columnas y los asignamos como tamaño preferente
         int[] anchos = {(anchoTabla*num[0])/100, (anchoTabla*num[1])/100, (anchoTabla*num[2])/100, (anchoTabla*num[3])/100, (anchoTabla*num[4])/100, (anchoTabla*num[5])/100, (anchoTabla*num[6])/100, (anchoTabla*num[7])/100, (anchoTabla*num[8])/100};        
    ...
    public void guardarTabla(){
       ...
       for (int i = 0; i < filas; i++) {
           //Obtenemos el partido en el ArrayList y le asignamos los nuevos datos
            PartidoRugby P = (PartidoRugby) J.partidos.get(i);
            ...
            P.setTriesL(validarEnterosTries(getModelo().getValueAt(i,6).toString().trim()));
            P.setTriesV(validarEnterosTries(getModelo().getValueAt(i,7).toString().trim()));
            P.setPista(limitarCadena(getModelo().getValueAt(i,8).toString().trim(), 30, false));
        }
    ...
    public void mostrarJornada(){
     ...
     /**
     * Recibe los tries en formato cadena y de ser incorrectos devuelve 0 (de lo contrario muestra en la clasificación -1)
     * Método especial par MetodosRugby
     * @param cadena
     * @return 
     */
    public int validarEnterosTries(String cadena){
        int numero;
        ...
        //Si la casilla está vacía ignoramos el chequeo, pues no hay información que comprobar
            ...
            return 0;//Devolvemos el número que será tratado por nuestro programa
    ...
    @Override
     public void cargarClasificacion(){
        //Textos para las columnas
        String[] columnasLocal = {"POS","COMPETIDOR", "PJ", "PG", "PE", "PP", "PF", "PC", "TF", "TC", "PTOS"};
        setColumnasClasf(columnasLocal);
        //Indicamos que las columnas de la clasificación no serán editables
        final boolean[] canEdit = new boolean [] {false, false, false, false, false, false, false, false, false, false, false};
        //Asignamos los nuevos anchos al array (los del archivo.txt)
        setNumc(getAnchoTabla(getNumc().length, CODIGO_ANCHO_CLASF));
        //Asignamos los anchos a las columnas
        int[] anchos = {(anchoTabla*numc[0])/100, (anchoTabla*numc[1])/100, (anchoTabla*numc[2])/100, (anchoTabla*numc[3])/100, (anchoTabla*numc[4])/100, (anchoTabla*numc[5])/100, (anchoTabla*numc[6])/100, (anchoTabla*numc[7])/100, (anchoTabla*numc[8])/100, (anchoTabla*numc[9])/100, (anchoTabla*numc[10])/100};
        ...
        renderClasf = new RenderRugbyClasf();
    ...
    @Override
     public void calcularClasificacion(){
         ...
         int golesL, golesV, triesL, triesV;
            ...
            for (int i = 0; i < filas; i++) {
                //Obtenemos el partido en el ArrayList y sus datos correspondientes
                 ...
                 for (EquipoRugby equipo: (ArrayList<EquipoRugby>)getCastEquipoLista().getEquipos()){                    
                     ...
                     asignarDatos(equipo, local, golesL, golesV, visitante, triesL, triesV);//Casteamos el Objeto recibido como Equipo de Futbol
                 }
             }//End for i
         }//End for j
    ...
    @Override
     ...
     /**
      * Método que asigna la puntuación, score y partidos... al equipo pasado 
     ...
     * @param triesV 
      */
     public void asignarDatos(EquipoRugby equipo, String local, int golesL, int golesV, String visitante, int triesL, int triesV){
         ...
         if(golesL < 0 || golesV < 0){
            ...
            if(equipo.getNombre().equalsIgnoreCase(local)){
               ...
               equipo.setPf(equipo.getPf() + golesL);
               equipo.setPc(equipo.getPc() + golesV);
               //Asignamos los tries al equipo local y le sumamos un pto. si los tries a favor son 4 o más
               equipo.setTf(equipo.getTf() + triesL);
               equipo.setTc(equipo.getTc() + triesV);
               if(triesL >= 4){
                   equipo.setPtos(equipo.getPtos() + 1);
               }
               ...
               //Sumamos los goles a favor y en contra y el partido jugado
               equipo.setPj(equipo.getPj() + 1);
               equipo.setPf(equipo.getPf() + golesV);
               equipo.setPc(equipo.getPc() + golesL);
               //Asignamos los tries al equipo visitante y le sumamos un pto. si los tries a favor son 4 o más
               equipo.setTf(equipo.getTf() + triesV);
               equipo.setTc(equipo.getTc() + triesL);
               if(triesV >= 4){
                   equipo.setPtos(equipo.getPtos() + 1);
               }
     ...
     /**
      * Método que ordena el ArrayList con el que mostraremos la clasificación (Rugby cuenta con ED)
      * Ordenamos en orden inverso a prioridad, primero por diferencia de tries, luego por diferencia de puntuación, y por último por puntos
      */
    @Override
     public void ordenarClasificacion(){
         //Le indicamos al comparador que el tipo de objeto a ordenar son Equipos
        Collections.sort(getCastEquipoLista().getEquipos(), new Comparator<EquipoRugby>() {
            @Override
            public int compare(EquipoRugby e1, EquipoRugby e2) {
                    // Aqui esta el truco, ahora comparamos p2 con p1 y no al reves como antes
                    return new Integer(e2.getTf() - e2.getTc()).compareTo(e1.getTf() - e1.getTc());
            }                                        
        });
    ...
    public void informeJornada(String rutaPDF, String nombreArchivo, boolean aplazados, ArrayList<Partido> partidosAplazados){
       ...
       if(aplazados){
        ...
        try {
            ...
            Logger.getLogger(MetodosRugby.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public void informeResultados(String rutaPDF, String nombreArchivo, boolean aplazados, ArrayList<Partido> partidosAplazados){
       ...
       String jornadaText = valueOf(jornada);//Pasamos a String el número de jornada
       String descansa = "";//Obtenemos el nombre del equipo que descansa        
       DataSourceRugbyResultados datasource = new DataSourceRugbyResultados();  
       if(aplazados){
            //Añadimos los partidos aplazados al datasource
             for (int i = 0; i < partidosAplazados.size(); i++) {
                PartidoRugby P = (PartidoRugby) partidosAplazados.get(i);
                datasource.addPartido(P);
             }
             jornadaText = "Aplz";//Cambiamo el nombre de la jornada a Aplazados
       }else{
        ...
        File archivoInforme = new File("src/informes/rugbyResultados.jasper");
        JasperReport reporte;
        parametros.put("nombreTorneo", nombreTorneo);
        parametros.put("jornada", jornadaText);
        //Si el número de equipos es impar añadimos el texto DESCANSA: y el nombre del equipo que descansa al informe, de lo contrario mejor que no aparezca
   ...
   /**
    ...
    public void informeClasificacion(String rutaPDF, String nombreArchivo){
        //lo primero de todo nos aseguramos de que ha guardado los datos de la tabla
       this.guardarTabla();
       ArrayList<EquipoRugby> equipos = castEquipoLista.getEquipos();
        DataSourceRugbyClasificacion datasource = new DataSourceRugbyClasificacion();  
        //Añadimos los partidos de la jornada al datasource
        for (EquipoRugby equipo : equipos) {
            EquipoRugby E = (EquipoRugby) equipo;
            //Comprobamos si descansa algún equipo y guardamos su nombre en la variable indicada
        ...
        File archivoInforme = new File("src/informes/rugbyClasificacion.jasper");
        JasperReport reporte;
        //Creamos un HashMap para poder utilizar los parámetros en el informe
        HashMap<String, Object> parametros = new HashMap<>();
        parametros.put("nombreTorneo", nombreTorneo);
