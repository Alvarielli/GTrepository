...
import datasources.DataSourceEliminatoriasFutbolResultados;
...
import renders.RenderEliminatoriaFutbol;
import java.util.Arrays;

/**
 * Válido para Fútbol, Basket, Fútbol Sala, Balonmano, Waterpolo, Hockey Patines, Hockey Hierba, Hockey Hielo, Rugby, Fútbol Americano, Ajedrez
 * @author Alvarielli
 */
public class MetodosEliminatoriaFutbol extends Metodos{
    ...
    private RenderEliminatoriaFutbol render;
    private int[] num = {10, 6, 16, 6, 6, 16, 10, 10, 20};// Definimos los anchos para las tablas en porcentaje (para que el usuario pueda modificarlas luego)
    ...
    private final int CODIGO_ANCHO = 308;
    private final int CODIGO_ALTO = 28;
    ...
    public MetodosEliminatoriaFutbol(DefaultTableModel modelo, int jornada, JTable TableJornada,JLabel LabelJornada, JLabel LabelTotalJornadas, 
    ...
    public void cargarTabla(int jornada){
        ...
        String[] columnasLocal = {"FECHA", "HORA", "EQUIPO LOCAL", "RES.L", "RES.V", "EQUIPO VISITANTE", "CAMPO", "GANADOR", "RESULTADO DEL DESEMPATE"};
        ...
        asignarGanador(TableJornada, TableJornada.getColumnModel().getColumn(7));
        //Le asignamos el render al calendario
        render = new RenderEliminatoriaFutbol();
    ...
    public void guardarTabla(){
       ...
       for (int i = 0; i < filas; i++) {
           //Obtenemos el partido en el ArrayList y le asignamos los nuevos datos
            PartidoEliminatoriaFutbol P = (PartidoEliminatoriaFutbol) J.partidos.get(i);
            ...
            P.setGanador(limitarCadena(getModelo().getValueAt(i,7).toString().trim(), 30, false));
            P.setDesempate(limitarCadena(getModelo().getValueAt(i,8).toString().trim(), 30, false));
            
            //Si el deporte seleccionado es baloncesto tenemos en cuenta que los equipos no puedan empatar (si bien siempre se podria haber dejado para el campo resultados del desempate)
            if(deporte == 1){
                ...
                if(P.getGolesL() == P.getGolesV() && !getModelo().getValueAt(i,3).toString().equalsIgnoreCase("") && !getModelo().getValueAt(i,4).toString().equalsIgnoreCase("")){
                    ...
                    if(!this.mensaje){//if esclusivamente para mostrar o no el mensaje
                         JOptionPane.showMessageDialog(TableJornada, "El resultado no es válido, los participantes no pueden empatar", "Error al insertar los datos de la tabla", JOptionPane.ERROR_MESSAGE, null);
                         this.mensaje = true;
                     }
                }
            }
    ...
    public void actualizarNuevaEliminatoriaIdaVuelta(){ 
        ...
        while(jor < getNumEliminatorias()){
            ...
            //Definimos el tamaño del Arraylist en caso de que la Eliminatoria sea la última y la opción tercerCuartoPuesto sea verdadera para poder añadir los equipos con posición 2,3
            for (PartidoEliminatoria partido :(ArrayList<PartidoEliminatoriaFutbol>) J.getListaPartidos())
                //Si el local o el visitante no tienen rival les asignamos como ganadores
                if(partido.getGanador().equalsIgnoreCase("LOCAL")){
                    getAuxEquipos().add(partido.getLocal());
                   //Si hay tercer y cuarto puesto y son semifinales añadimos también al Arraylist el equipo derrotado
            ...
            int indice = 0;//índice auxiliar que nos ayudará a poner cada equipo en su hueco correspondiente
            for (PartidoEliminatoria partidoSig :(ArrayList<PartidoEliminatoriaFutbol>) Jsiguiente.getListaPartidos()){
                //Si hay tercer y cuarto puesto y son semifinales modificamos la forma de recoger los equipos, pues ahora están también los que han perdido en el ArrayList
                    if(jor == (getNumEliminatorias()-2)  && isTercerCuartoPuesto() == true){
                        partidoSig.setLocal(getAuxEquipos().get(indice));
                        partidoSig.setVisitante(getAuxEquipos().get(indice+2));
            ...
            getAuxEquipos().clear();//vaciamos el arrayList auxiliar
            //Definimos el tamaño del Arraylist en caso de que la Eliminatoria sea la última y la opción tercerCuartoPuesto sea verdadera para poder añadir los equipos con posición 2,3
            for (PartidoEliminatoria partido :(ArrayList<PartidoEliminatoriaFutbol>) J2.getListaPartidos()){
                    getAuxEquipos().add(partido.getVisitante());
                    getAuxEquipos().add(partido.getLocal());
            }
     ...
     public void guardarXML(String ruta, String nombreArchivo){
        ...
        for (int i = 0; i < num_jornadas; i++) {          
            ...
            Jornada J = getCalendario().jornadas.get(i);//i porque es un índice
                ...
                //Ahora usamos el for-each y obtenemos la lista de partidos de este objeto Jornada
                for (PartidoEliminatoriaFutbol partido :(ArrayList<PartidoEliminatoriaFutbol>) J.getListaPartidos()){ 
    ...
    public void informeJornada(String rutaPDF, String nombreArchivo, boolean aplazados, ArrayList<Partido> partidosAplazados){
       ...
       if(aplazados){
        ...
        File archivoInforme = new File("src/informes/eliFutbolJornada.jasper");
        ...
        try {
            ...
            Logger.getLogger(MetodosEliminatoriaFutbol.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public void informeResultados(String rutaPDF, String nombreArchivo, boolean aplazados, ArrayList<Partido> partidosAplazados){
       ...
       HashMap<String, Object> parametros = new HashMap<>();
       String descansa = "";//Obtenemos el nombre del equipo que descansa        
       DataSourceEliminatoriasFutbolResultados datasource = new DataSourceEliminatoriasFutbolResultados();  
       if(aplazados){
            //Añadimos los partidos aplazados al datasource
             for (int i = 0; i < partidosAplazados.size(); i++) {
                PartidoEliminatoria P = (PartidoEliminatoria) partidosAplazados.get(i);
                datasource.addPartido(P);
             }
       }else{
            Jornada J = getCalendario().jornadas.get(getJornada() - 1);//coge la jornada como variable global, -1 porque es un índice
             //Añadimos los partidos de la jornada al datasource
                 for (int i = 0; i < J.getListaPartidos().size(); i++) {
                     PartidoEliminatoria P = (PartidoEliminatoria) J.partidos.get(i);
                     //Comprobamos si descansa algún equipo y guardamos su nombre en la variable indicada
                     if(P.getLocal().equalsIgnoreCase("EquipoFantasma")){
                         descansa = P.getVisitante();
                     }else if(P.getVisitante().equalsIgnoreCase("EquipoFantasma")){
        ...
        File archivoInforme = new File("src/informes/eliFutbolResultados.jasper");
        JasperReport reporte;
        parametros.put("nombreTorneo", nombreTorneo);
        if(aplazados){
            parametros.put("jornada", "Partidos Aplazados");
    ...
    public void informeCalendario(String rutaPDF, String nombreArchivo){
        ...
        try {
            ...
            if(num_partidos > 40){//40 es el limite de campos que le hemos puesto al page break en el futbolCalendario.jasper
                if(mostrar){
                    archivoInforme = new File("src/informes/eliFutbolCalendarioBIG.jasper");
                }else{
                    archivoInforme = new File("src/informes/eliFutbolCalendarioBIGNo.jasper");//No muestra la vuelta de la ronda final
                }//end if
            }else{
                if(mostrar){
                    archivoInforme = new File("src/informes/eliFutbolCalendario.jasper");
                }else{
                    archivoInforme = new File("src/informes/eliFutbolCalendarioNo.jasper");//No muestra la vuelta de la ronda final
                }//end if              
            }
            JasperReport reporte;
