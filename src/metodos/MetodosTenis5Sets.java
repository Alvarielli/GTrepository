...
/**
 * Válido también para Tenis de Mesa
 * @author Alvarielli
 */
public class MetodosTenis5Sets extends Metodos{
    ...
    private final int CODIGO_ANCHO = 193;
    private final int CODIGO_ALTO = 20;
    private final int CODIGO_ANCHO_CLASF = 208;
    private final int CODIGO_ALTO_CLASF = 21;
    
    //En el constructor de clase le pasamos todo lo requerido para esta clase desde Principal
    public MetodosTenis5Sets(DefaultTableModel modelo, DefaultTableModel clasificacion, int jornada, JTable TableJornada, JTable TableClasificacion, JLabel LabelEquipoDescansa,
    ...
    @Override
     public void cargarClasificacion(){
        //Textos para las columnas
        String[] columnasLocal = {"POSICIÓN","COMPETIDOR", "PJ", "PG", "PP", "SF", "SC", "JF", "JC"};
    ...
    @Override
      ...
      * @param visitante     
      */
     public void asignarDatos(EquipoTenis equipo, String local, int set1L, int set1V, int set2L, int set2V, int set3L, int set3V, int set4L, int set4V, int set5L, int set5V, String visitante){
        int auxL = 0;
        ...
        //Primero comprobamos que los resultados son válidos, si es -1
         if(set1L < 0 || set1V < 0){
             ...
             //Si el local ha ganado los tres primeros sets o el visistante ignoramos el tercero
             ...
             //Ahora tenemos que establecer la diferencia de ptos. que acumula cada equipo
                if(equipo.getNombre().equalsIgnoreCase(local)){
                    ...
                    if(set4L > set4V){
                        ...
                        equipo.setSf(equipo.getSf() + 1);
                        equipo.setJf(equipo.getJf() + set4L);
                        equipo.setJc(equipo.getJc() + set4V);
                        auxL += 1; 
                    }else if(set4L < set4V){
                        //Sumamos a partidos perdidos
                        equipo.setSc(equipo.getSc() + 1);
                    ...
                    if(set4V > set4L){
                        //Sólo si se suma si se juega
                        equipo.setSf(equipo.getSf() + 1);
                        equipo.setJf(equipo.getJf() + set4V);
                        equipo.setJc(equipo.getJc() + set4L);
                        auxV += 1; 
                    }else if(set4V < set4L){
                        //Sumamos a partidos perdidos
                        equipo.setSc(equipo.getSc() + 1);
          ...
          if(auxL == 3){
              ...
              if(equipo.getNombre().equalsIgnoreCase(local)){
                ...
                if(set5L > set5V){
                    //Sólo si se suma si se juega
                    equipo.setSf(equipo.getSf() + 1);
                    equipo.setJf(equipo.getJf() + set5L);
                    equipo.setJc(equipo.getJc() + set5V);
                    equipo.setPg(equipo.getPg() + 1);
                    equipo.setPj(equipo.getPj() + 1);
                }else if(set5L < set5V){
                    //Sumamos a partidos perdidos
                ...
                if(set5V > set5L){
                    //Sólo si se suma si se juega
                    equipo.setSf(equipo.getSf() + 1);
                    equipo.setJf(equipo.getJf() + set5V);
                    equipo.setJc(equipo.getJc() + set5L);
                    equipo.setPg(equipo.getPg() + 1);
                    equipo.setPj(equipo.getPj() + 1); 
                }else if(set5V < set5L){
                    //Sumamos a partidos perdidos
    ...
    public void informeJornada(String rutaPDF, String nombreArchivo, boolean aplazados, ArrayList<Partido> partidosAplazados){
       ...
       if(aplazados){
        ...
        try {
            ...
            Logger.getLogger(MetodosTenis5Sets.class.getName()).log(Level.SEVERE, null, ex);
        }
